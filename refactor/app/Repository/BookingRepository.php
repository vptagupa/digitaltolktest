...
use DTApi\Helpers\GlobalHelper;
use DTApi\Services\BookingServices;
use DTApi\Services\Notification;
...
{   
    ...
    use BookingServices, Notification; 
...
//        if (in_array($data['status'], ['pending', 'assigned']) && date('Y-m-d H:i:s') <= $job->due) {
        ...
        if ($data['status'] == 'pending') {
            ...
            $this->sendNotificationTranslatorsendNotificationTranslator($job, $job_data, '*');   // send Push all sutiable translators
        ...
        if (!Job::isTranslatorAlreadyBooked($job_id, $cuser->id, $job->due)) {
            if ($job->status == 'pending' && Job::insertTranslatorJobRel($cuser->id, $job_id)) {
                ...
                if ($this->isNeedToSendPush($user->id)) {
                    ...
                    $this->sendNotificationTranslatorsendPushNotificationToSpecificUsers($users_array, $job_id, $data, $msg_text, $this->isNeedToDelayPush($user->id));
                }
        ...
        if ($cuser->is('customer')) {
            ...
            if ($translator) {
                ...
                if ($this->isNeedToSendPush($translator->id)) {
                    ...
                    $this->sendNotificationTranslatorsendPushNotificationToSpecificUsers($users_array, $job_id, $data, $msg_text, $this->isNeedToDelayPush($translator->id));// send Session Cancel Push to Translaotor
                }
            }
        } else {
            if ($job->due->diffInHours(Carbon::now()) > 24) {
                ...
                if ($customer) {
                    ...
                    if ($this->isNeedToSendPush($customer->id)) {
                        ...
                        $this->sendNotificationTranslatorsendPushNotificationToSpecificUsers($users_array, $job_id, $data, $msg_text, $this->isNeedToDelayPush($customer->id));     // send Session Cancel Push to customer
                    }
                }
                $job->status = 'pending';
                $job->created_at = date('Y-m-d H:i:s');
...
//                Event::fire(new JobWasCanceled($job));
                ...
                $this->sendNotificationTranslatorsendNotificationTranslator($job, $data, $translator->id);   // send Push all sutiable translators
                $response['status'] = 'success';
            } else {
                $response['status'] = 'fail';
